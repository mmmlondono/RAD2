...
vcfR<-read.vcfR("eurytoma_filtered.vcf")
popmap <- read.csv("pop_list_site.csv")
...
fst <- read.csv("eury_fst.csv")

popNames(snps)

# Desired order of labels
colnames(fst) <- c(".", "02-S","04-S","05-S", "07-C", "09-C", "10-C" ,"15-S")
rownames(fst) <- c("02-S","04-S","05-S", "07-C", "09-C", "10-C" ,"15-S")
fst <- fst[,2:8]
#lab_order2 = c(",4",)
...
ggplot(data = fst.df, aes(x = Site1, y = Site2, fill = Fst))+
  ...
  theme(axis.text = element_text(colour = "black", size = 20, face = "bold.italic"),
