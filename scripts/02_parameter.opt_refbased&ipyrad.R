...
####reference based####
##Mean depth
var_depth <- read_delim("rbrush/reference_based/vcftools_ref/ref_sitedepth.ldepth.mean", delim = "\t",
...
a
##Variant missingness
var_miss <- read_delim("rbrush/reference_based/vcftools_ref/ref_miss_site.lmiss", delim = "\t",
...
##Minor alelle frecuencies
var_freq <- read_delim("rbrush/reference_based/vcftools_ref/ref_alleles.frq", delim = "\t",
...
##Mean depth per individual
ind_depth <- read_delim("rbrush/reference_based/vcftools_ref/ref_depth.idepth", delim = "\t",
                        col_names = c("ind", "nsites", "depth"), skip = 1)
d <- ggplot(ind_depth, aes(depth)) + geom_histogram(fill = "dodgerblue1", colour = "black", alpha = 0.3)
d + theme_light()

##Proportion of missing data per individual
ind_miss  <- read_delim("rbrush/reference_based/vcftools_ref/ref_miss_indiv.imiss", delim = "\t",
                        col_names = c("ind", "ndata", "nfiltered", "nmiss", "fmiss"), skip = 1)
e <- ggplot(ind_miss, aes(fmiss)) + geom_histogram(fill = "dodgerblue1", colour = "black", alpha = 0.3)
e + theme_light()

####ipyrad####
var_depth1 <- read_delim("ipyrad/vcftools_ipyrad/ipy_sitedepth.ldepth.mean", delim = "\t",
                         col_names = c("chr", "pos", "mean_depth", "var_depth"), skip =1)
...
var_miss1 <- read_delim("ipyrad/vcftools_ipyrad/ipy_miss_site.lmiss", delim = "\t",
                        col_names = c("chr", "pos", "nchr", "nfiltered", "nmiss","fmiss"), skip = 1)
b1 <- ggplot(var_miss1, aes(fmiss)) + geom_density(fill = "dodgerblue1", colour = "black", alpha = 0.3)
b1 + theme_light()
summary(var_miss1$fmiss)
var_freq1 <- read_delim("ipyrad/vcftools_ipyrad/ipy_alleles.frq", delim = "\t",
                        col_names = c("chr", "pos", "nalleles", "nchr", "a1", "a2"), skip = 1)
...
ind_depth1 <- read_delim("ipyrad/vcftools_ipyrad/ipy_depth.idepth", delim = "\t",
                         col_names = c("ind", "nsites", "depth"), skip = 1)
d1 <- ggplot(ind_depth1, aes(depth)) + geom_histogram(fill = "dodgerblue1", colour = "black", alpha = 0.3)
d1 + theme_light()
ind_miss1  <- read_delim("ipyrad/vcftools_ipyrad/ipy_miss_indiv.imiss", delim = "\t",
                         col_names = c("ind", "ndata", "nfiltered", "nmiss", "fmiss"), skip = 1)
