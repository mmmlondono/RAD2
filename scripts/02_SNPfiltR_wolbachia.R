####READ ME####
# The purpose of this script is to run SNPfiltR protocol for wolbachia project dataset
#https://devonderaad.github.io/SNPfiltR/#our-package-contains-a-convenient-wrapper-functions-that-can-filter-based-on-minor-allele-count-mac-and-streamline-investigation-of-the-effects-of-various-filtering-parameters-on-sample-clustering-patterns
####libraries####
library(SNPfiltR)
library(devtools)
...
#This means that SNPs with fewer than 2 occurrences of the minor allele will be removed from the VCF
...
vcfR<-missing_by_sample(vcfR=vcfR, cutoff = .90)
...
miss<-assess_missing_data_pca(vcfR=vcfR, popmap = popmap,
                              thresholds = c(.70, .75,.85,.95), clustering = FALSE)
...
vcfR::write.vcf(vcfR.thin, "wolbachia.thinned.vcf.gz")

#I can write out the vcf with singletons removed as well:
